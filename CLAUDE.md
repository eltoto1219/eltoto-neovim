...
- `lua/eltoto/dictation.lua`: `<leader>v` voice dictation (arecord + faster-whisper from the repo `.venv`); transcription runs in a persistent `scripts/transcribe.py --serve` job (one wav path in and one JSON response out per line) so the model loads once per nvim session; model auto-picks medium on GPU / base on CPU (`M.model = nil` = auto).
