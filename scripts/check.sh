...
nvim --headless "+lua for _, cmd in ipairs({':AIStatus', ':EltotoHealth', ':Shortcuts', ':ShortcutsSync', ':TerminalConfig', ':TerminalRename', ':TerminalProcesses', ':TerminalProcessNew', ':TerminalProcessKill', ':TerminalProcessAttachLast'}) do assert(vim.fn.exists(cmd) == 2, cmd) end" "+qa"
